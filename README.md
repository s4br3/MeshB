MeshB: A boolean operations solver that hopes to match up to predicate-based counterparts with floating-point arithmetic, built for researchers trying to model electromagnetic flow.
...
Build: NO requirements whatsoever. The only libraries used are Shashwat Sharma's OpenBEM finite-element method solver (for TriangleMesh<3> compatibility) and Artem Ogre's CDT library,
